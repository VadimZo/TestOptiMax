const initialState = {
  ...
  const getTotalPrice=(arr)=>Object.keys(arr).map(key=>arr[key].totalPrice).reduce((sum,el)=>sum+el,0);
  const getTotalCount=(arr)=>Object.keys(arr).map(key=>arr[key].items.length).reduce((count,el)=>count+el,0);
